    ...
    <>
      ...
      {phase === 'canvas' && elements.length === 0 && (
        <div className="hint-box">
          ...
          Klikej na plátno pro body exterior ringu.<br />
          Po uzavření můžeš polygon dokončit nebo přidat interior ring.<br />
        ...
        <div key={el.id} className={`element-item ${selectedId === el.id ? 'selected' : ''}`} onClick={() => setSelectedId(el.id)}>
          <div className="el-info">
            ...
            <div className="el-points">
              {el.geometry.exterior.length} ext · {el.geometry.holes.length} holes · {el.attributes.length} attr
