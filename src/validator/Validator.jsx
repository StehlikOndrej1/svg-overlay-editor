...
import { createGeometry, parsePathRings } from '../shared/lib/geometry.js';
...
function parseValidatorElements(text) {
  ...
  return Array.from(shapes).map(shape => {
    ...
    if (tag === 'polygon') {
      const exterior = (shape.getAttribute('points') || '').split(/\s+/).filter(Boolean).map(pair => {
    ...
    return { id: shape.getAttribute('data-overlay-id'), tag, geometry, attrs };
  });
}
            ...
            {!image || !svgContent ? (
              ...
              <div style={{ animation: 'fadeIn 0.2s ease' }}>
                ...
                <div className="form-group">
                  ...
                  <div className="form-input" style={{ background: 'var(--bg-tertiary)', fontSize: 11, lineHeight: 1.5 }}>
                    {selectedEl.geometry.exterior.length} bodů v exterioru<br />
                    {selectedEl.geometry.holes.length} interior rings
                  </div>
                </div>
              ...
              <>
                ...
                {svgElements.map(el => (
                  <div key={el.id} className="element-item" onClick={() => setSelectedEl(el)}>
                    <div className="el-info"><div className="el-id">{el.id}</div><div className="el-points">{el.geometry.holes.length} holes</div></div>
